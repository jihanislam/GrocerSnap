...
if (isset($_GET['pending'])) {
    $orderId = intval($_GET['pending']); // Sanitize input to prevent SQL injection

    // Update the orderStatus in the database
    $updateQuery = "UPDATE orders SET orderStatus = 'pending' WHERE id = ?";
    ...
    if ($status) {
        ...
        if ($status->execute()) {
            echo "<script>
                        window.location.href = 'cOrder.php?success=Order Status Updated';
                    </script>";
            exit();
        } else {
            echo "<script>
                        window.location.href = 'cOrder.php?error=Failed to confirm Order';
                    </script>";
            exit();
        }
    } else {
        echo "<script>
                window.location.href = 'cOrder.php?error=Database-error';
            </script>";
        exit();
    }
}
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                echo '<td>
                        ...
                        <form action="cOrder.php" method="GET" style="display:inline;">
                            <input type="hidden" name="pending" value="'.$row['order_id'].'">
